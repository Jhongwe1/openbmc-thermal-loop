...
這裡有**兩張不同的圖**,它們回答的不是同一個問題。
混為一談會讓「分層」這個詞失去意義 —— 一張講產品的時間尺度,一張講驗證的成本。
...
> ⚠️ **這不是「違反奈奎斯特取樣定理」。** 那是訊號重建的定理,不是控制迴路穩定性的
> 判準;判準是**相位裕度**與**死區時間佔比**。兩者結論方向雖然一致,依據完全不同。
...
只在本機模擬,則沒有任何證據說明結論在真的 OpenBMC 上還成立 ——
模擬跟真 daemon 之間差的是整個 D-Bus、設定解析與排程。

**所以要分層:每一層各自回答一個不同的問題,而迭代成本差六個數量級。**
...
上面兩張圖把佈局固定住了,而**「不看圖能不能重畫一次」才是理解這條鏈的檢驗** ——
這一步還沒做。手繪稿完成後放進 `docs/` 並在此連結。
...
route (a) 的設定仍留在 git 歷史裡(commit `52f84c8`),兩條路線的實作與取捨都對照得到。
